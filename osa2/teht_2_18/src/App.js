...
//        id: persons.length + 1,
    ...
    if (persons.some(person => person.name === newName)) {
      if(window.confirm(`Person ${newName} is already in the list, do you want to replace the old number?`)){
        const updatingPerson = {...persons.find(p => p.name === newName), number: newNumber};
        personService.updatePerson(updatingPerson)
        .then(newData => setPersons(persons.map(p => p.name !== newData.name ? p : newData)))
      }
    } else {
